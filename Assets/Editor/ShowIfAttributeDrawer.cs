...
public class ShowIfAttributeDrawer : PropertyDrawer
{
  ...
  BindingFlags.NonPublic | BindingFlags.Public)
    ...
    private bool MeetsConditions(SerializedProperty property)
    {
        ...
        {
            bool met;
            if (showIfAttribute.Operator == ShowConditions.ConditionOperator.And)
            {
                met = true;
                 ...
                 label)
    {
        ...
        if (!meetsCondition && showIfAttribute.Action ==
                                      ShowConditions.ActionOnConditionFail.DontDraw)
            return 0;
           ...
           label)
    {
        ...
        var showIfAttribute = this.attribute as ShowIfAttribute;
        if (showIfAttribute.Action == ShowConditions.ActionOnConditionFail.DontDraw)
        {
            return;
        }
        else if (showIfAttribute.Action == ShowConditions.ActionOnConditionFail.JustDisable)
        {
